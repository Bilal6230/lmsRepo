        ...
        <table class="table eTable">
            ...
            <tbody>
                ...
                @foreach ($enroll_students as $enroll_student)
                    ...
                    <tr>
                        <td class="student_name_{{ $enroll_student->user_id }}" >{{ $student_details->name }}</td>
                        <td>
                            <input class="form-control eForm-control student_mark_{{ $enroll_student->user_id }} " type="number"
                                id="mark-{{ $enroll_student->user_id }}" name="mark" placeholder="mark"
                                min="0" value="{{ $user_marks }}" required
                                onchange="get_grade(this.value, this.id)">
                        </td>
                        <td>
                            <span
                                id="grade-for-mark-{{ $enroll_student->user_id }}" class="student_grade_{{ $enroll_student->user_id }}">{{ get_grade($user_marks) }}</span>
                        </td>
                        <td>
                            <input class="form-control eForm-control student_coment_{{ $enroll_student->user_id }}" type="text"
                                id="comment-{{ $enroll_student->user_id }}" name="comment" placeholder="comment"
                                value="{{ $comment }}" required>
                        </td>
                        <td class="text-center">
                            <button class="btn btn-success" type="button"
                                onclick="mark_update('{{ $enroll_student->user_id }}')">
                                <i class="bi bi-check2-circle"></i>
                            </button>
                        </td>
                        <td>
                            <!-- Button to trigger form submission --> 
                            <button class="btn btn-success 2ai" data-id="{{ $enroll_student->user_id }}" type="submit">2AI</button>
                        </td>
